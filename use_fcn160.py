...
def fcn_pre(img_cv=0,filter_size=0,image_dir="",back_dir=""):  #filter_size0,3,5,7
    if len(image_dir)>2:
        img_cv = cv2.imread(image_dir)
    img = cv2.resize(img_cv, (160, 160))   # [160, 160, 3]
    ...
    fcn_model =torch.load('checkpoints/model480.pkl')
    fcn_model.eval()
    pre=fcn_model(im)
    ...
    for i in range(img2.shape[0]):    ##黑底换白底
        for j in range(img2.shape[1]):
            if img1[i][j][0]==0:
                merged[i][j][0:3] = 0
                merged[i][j][3] = 0
    lucency_img = cv2.medianBlur(merged, 3)  # 中值滤波函数  效果好
    mask = img1
    if filter_size!=0:
...
if __name__ == '__main__':
    ...
    fcn_img=fcn_pre(img_cv=image,filter_size=7,back_dir="test/17.jpg")   #背景图片
