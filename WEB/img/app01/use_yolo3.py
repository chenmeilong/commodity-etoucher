import torch
import sys
sys.path.append('D:\Wayne\Desktop/alibaba\FCN\yolov3_master')
...
cfg='static/yolov3/cfg/yolov3-tiny.cfg'
data='static/yolov3/data/bag.data'
weights='static/yolov3/weights/best.pt'
img_size=416
conf_thres=0.5    #conf_thres, nms_thres: 目标检测置信度，非极大抑制阈值
nms_thres=0.5
device="cuda:0"
...
def detect(source):      #
    ...
    for path, img, im0s, vid_cap in dataset:
        img = torch.from_numpy(img).to(device)    ######？？？？？？？送进去的图片大小不一样??????????
        if img.ndimension() == 3:
