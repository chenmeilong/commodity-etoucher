...
def trans_style(img_cv=0,image_dir="",style_dir="",style=2):
    ...
    if len(style_dir)>1:
        ...
        if style==1:
            style_img = load_img('style/style1.jpg')
        elif style==2:
            style_img = load_img('style/style2.jpg')
        elif style==3:
            style_img = load_img('style/style3.jpg')
...
if __name__ == '__main__':            #1简约 2古典 3水墨
    image = cv2.imread("D:\Wayne\Desktop/alibaba\FCN\data/test/13.jpg")
    out_style=trans_style(img_cv=image,style_dir="style/style2.jpg")       # 不填style_dir 为 style=1,2,3    填完路径为对应路径的jpg图片
